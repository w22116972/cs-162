...
/// Write byte literal b"\r\n") to s and handle errors
/// ? is used to propagate errors, if no error then return Ok(()), else return any `Err` value to the calling code
/// Result<()> is a type alias for Result<(), Error>, which is a common pattern for functions that return nothing on success and something (an Error) on failure.
...
/// Return mime type based on file extension found by path
/// Args:
///  path: path to file to find file extension
pub fn get_mime_type(path: &str) -> &'static str {
    // Option either returns Some(true) or None (false)
    if let Some(extension) = file_extension(path) {
        match extension {
            "html" | "htm" => "text/html",
            "jpg" | "jpeg" => "image/jpeg",
            "png" => "image/png",
    ...
    } else { "text/plain" } // if file not exist, return text/plain
}
